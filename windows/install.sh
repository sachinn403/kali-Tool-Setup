...
BASE_DIR="/opt/windows"
TOOLS_DIR="tools"
LOG_FILE="/tmp/windows_tools_install_$(date +%F_%H-%M-%S).log"
...
DEPENDENCIES=("curl" "git" "unzip" "python3" "python3-pip" "dos2unix" "wine")
MISSING_DEPS=()
...
declare -a CATEGORIES=(
  "Enumeration" "Exploitation" "Utilities"
)
...
process_tool_file() {
  ...
  while IFS=' ' read -r name url dest_subfolder || [[ -n "$name" ]]; do
    ...
    # Handle file downloads
      ...
      if [[ "$name" == *.zip || "$name" == *.exe || "$name" == *.ps1 ]]; then
...
log "[✓] Windows tools installation complete."
